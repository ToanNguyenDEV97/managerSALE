...
const LoginPage: React.FC<LoginPageProps> = ({ onGoToRegister }) => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 p-4">
            {/* ... (Giữ nguyên phần giao diện Header và Form) ... */}
            <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden transform transition-all hover:shadow-2xl animate-fade-in">
                ...
                <div className="p-8 pt-10">
                    <form onSubmit={handleLogin} className="space-y-6">
                        {/* ... (Các input Email, Password, Checkbox giữ nguyên) ... */}
                         <div className="group">
                        ...
                        <div className="group">
                         ...
                         <div className="flex items-center justify-between">
                            ...
                            <div className="text-sm">
                                <a href="#" className="font-medium text-primary-600 hover:text-primary-500">Quên mật khẩu?</a>
                            </div>
                        </div>
                        ...
                        <button type="submit" disabled={loading} className={`w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-lg shadow-md text-sm font-bold text-white transition-all duration-200 ${loading ? 'bg-primary-400 cursor-wait' : 'bg-primary-600 hover:bg-primary-700 hover:shadow-lg hover:-translate-y-0.5'}`}>
                            {loading ? <><FiLoader className="animate-spin -ml-1 mr-2 h-5 w-5" /> Đang xử lý...</> : <><span className="mr-2">Đăng nhập</span> <FiLogIn className="h-5 w-5" /></>}
                        </button>
                    </form>

                    {/* Footer Register - ĐỔI LOGIC TẠI ĐÂY */}
                    <div className="mt-8 pt-6 border-t border-slate-100 text-center">
                        <p className="text-sm text-slate-600 mb-2">Chưa có tài khoản?</p>
                        <button
                            type="button"
                            // [SỬA] Dùng navigate thay vì props cũ
